...
function deriveKey(userPassword: string, userId: string) {
  const salt = Buffer.from(userId, "utf8");
  return crypto.scryptSync(userPassword, salt, 32);
}

function encryptPassword(plain: string, key: Buffer) {
...
function decryptPassword(encrypted: Buffer, key: Buffer, iv: Buffer, tag: Buffer) {
...
function toPublic(entry: VaultEntry) {
  return {
    id: entry.id,
...
const revealAttempts = new Map<string, AttemptState>();
...
vaultRouter.put("/:id", requireAuth, async (req, res) => {
  ...
  if (accountIdentifier !== undefined) entry.accountIdentifier = accountIdentifier;
